...
// 定义服务器传输接口，而不是使用声明合并
interface ServerTransportHandle {
  receiveRequest(id: string, method: string, params: any): Promise<void>;
}
  
interface ServerTransport {
  connect(handle: ServerTransportHandle): Promise<void>;
  sendResponse(id: string, result: any): Promise<void>;
  sendErrorResponse(id: string, error: any): Promise<void>;
  close(): void;
}
...
// 验证JSON-RPC请求格式
function validateJsonRpcRequest(request: any): { valid: boolean; error?: { code: number; message: string } } {
  // 必须是对象
...
// 验证方法调用参数
function validateMethodParams(method: string, params: any): { valid: boolean; error?: { code: number; message: string } } {
  // 工具调用必须有params对象
...
class SSEServerTransport implements ServerTransport {
  private handle?: ServerTransportHandle;
  ...
  // 连接到MCP服务器
  async connect(handle: ServerTransportHandle): Promise<void> {
    this.handle = handle;
    this.connected = true;
  ...
  // 发送响应到客户端
  async sendResponse(id: string, result: any): Promise<void> {
  ...
  // 发送错误响应到客户端
  async sendErrorResponse(id: string, error: any): Promise<void> {
    if (!this.res || this.res.closed) {
  ...
  // 获取处理器，用于访问
  getHandle(): ServerTransportHandle | undefined {
    return this.handle;
  }
  ...
  // 处理和验证接收到的请求
  async processRequest(request: any): Promise<boolean> {
    if (!this.handle) {
    ...
    try {
      // 处理请求
      await this.handle.receiveRequest(
        request.id,
        request.method,
        request.params || {}
      );
...
export class SSEServer {
  ...
  private mcpServer?: any; // 添加MCP服务器引用
  private server?: any; // 添加HTTP服务器引用
  ...
  constructor(config: SSEServerConfig) {
    ...
    this.app.get('/sse', (req, res) => {
      ...
      if (this.mcpServer) {
        ...
        // 连接服务器
        this.mcpServer.connect(transport as any).catch((error: any) => {
  ...
  // 为连接创建或获取传输实例
  getTransport(res: Response, connectionId?: string): SSEServerTransport {
