...
export async function createPerformanceServer(
  ...
  if (mergedConfig.prewarmBrowser) {
    try {
      console.log('预热浏览器实例中...');
      await prewarmBrowser();
      console.log('浏览器实例预热完成');
    } catch (error) {
      console.warn('浏览器预热失败，将在首次请求时初始化:', error);
    }
  }
      ...
      try {
        ...
        return {
          content: [
            {
              type: 'text',
              text: `获取截图失败: ${error.message || '未知错误'}`,
            },
          ],
          isError: true,
        };
      ...
      try {
        if (transportMode === 'stdio') {
          ...
          try {
            ...
            console.log(`性能分析MCP服务器(SSE)启动成功，端口: ${mergedConfig.ssePort}`);
            console.log(`可以通过以下地址访问SSE端点: http://localhost:${mergedConfig.ssePort}/sse`);
          } catch (error: any) {
