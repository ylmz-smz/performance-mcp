...
async function getScreenshotHandler({ sessionId }: { sessionId: string }) {
  try {
    const session = getSession(sessionId);
    if (!session) {
      return { content: [{ type: 'text' as const, text: `找不到会话ID: ${sessionId}` }], isError: true };
    }
    if (!session.screenshot) {
      return { content: [{ type: 'text' as const, text: `该会话没有保存截图` }], isError: true };
    }
    const screenshotData = await getSessionScreenshot(sessionId);
    if (!screenshotData) {
      return { content: [{ type: 'text' as const, text: `无法加载截图数据` }], isError: true };
    }
    return {
      content: [{
        type: 'image' as const,
        data: screenshotData,
        mimeType: (session.screenshot.format === 'png' ? 'image/png' : 'image/jpeg') as string,
      }],
    };
  } catch (error: any) {
    process.stderr.write(`获取截图失败: ${error}\n`);
    return { content: [{ type: 'text' as const, text: `获取截图失败: ${error.message || '未知错误'}` }], isError: true };
  }
}
...
interface PerformanceMcpServerConfig {
  ...
  maxRetries: number;
  retryDelay: number;
  prewarmBrowser: boolean;
}

// 默认配置
const DEFAULT_CONFIG: PerformanceMcpServerConfig = {
...
export async function createPerformanceServer(
  config: Partial<PerformanceMcpServerConfig> = {},
  transportMode: TransportMode = 'stdio'
): Promise<{ server: McpServer; start: () => Promise<void> }> {
  // 合并配置，确保所有字段都有默认值
  const mergedConfig: PerformanceMcpServerConfig = { 
    ...DEFAULT_CONFIG, 
    ...config,
