...
import type { PerformanceMetrics, ResourceStat, SlowResource } from '../types/performance.js';
  ...
  async getBrowser(): Promise<Browser> {
    ...
    if (this.instances.length >= config.browserPool.maxInstances) {
      throw new McpError(
        ...
        `已达到最大浏览器实例数限制(${config.browserPool.maxInstances})`,
        { currentCount: this.instances.length }
      );
    }
...
function isUrlAllowed(url: string): boolean {
  ...
  try {
    ...
    // 检查黑名单
    if (config.urlFiltering.blacklist.some((domain: string) => hostname === domain || hostname.endsWith(`.${domain}`))) {
      return false;
    }
    
    // 检查白名单(如果有设置)
    if (config.urlFiltering.whitelist.length > 0) {
      return config.urlFiltering.whitelist.some((domain: string) => hostname === domain || hostname.endsWith(`.${domain}`));
    }
 ...
 * 访问页面并返回Page对象
 */
export async function visitPage(browserOrUrl: Browser | string, urlOrTimeout?: string | number, timeoutArg?: number): Promise<Page | { page: Page, browser: Browser }> {
  // 如果第一个参数是Browser对象
  if (typeof browserOrUrl !== 'string') {
    const browser = browserOrUrl;
    const url = urlOrTimeout as string;
      ...
      try {
        ...
        if (error.name === 'TimeoutError') {
          throw new McpError(
            ...
            `页面加载超时(${actualTimeout}ms)`,
            { url, timeout: actualTimeout }
          );
        }
 ...
 * 模拟真实用户回访时的缓存命中场景
 */
export async function visitPageWithWarmCache(
  browser: Browser,
  url: string,
  timeout?: number
): Promise<Page> {
  ...
  try {
    ...
    const resources = await page.evaluate(() => {
      ...
      const result = {
        ...
        slowest: [] as Array<{ url: string, duration: number, size: number, type: string }>,
        networkQuality: {
          http2Count: 0,
          http1Count: 0,
          http1Resources: [] as string[],
          compressedCount: 0,
          uncompressedCount: 0,
          uncompressedResources: [] as Array<{ url: string; size: number; type: string }>,
          totalTtfb: 0,
          ttfbCount: 0,
          highTtfbCount: 0,
          highTtfbResources: [] as Array<{ url: string; ttfb: number }>,
          cacheHitCount: 0,
          cacheMissCount: 0,
        }
      };
    ...
    // 组装完整的性能指标数据
    const resourceStats: ResourceStat[] = Object.entries(resources.byType).map(([type, stats]) => ({
      type,
      count: stats.count,
      size: stats.size
    }));
    
    const slowestResources: SlowResource[] = resources.slowest.map((res: any) => ({
      url: res.url,
 ...
 * 收集Core Web Vitals指标
 */
async function collectWebVitals(page: Page): Promise<any> {
  // 注入web-vitals库
  await page.evaluate(() => {
 ...
 * 保存页面截图
 */
export async function saveScreenshot(page: Page, sessionId: string): Promise<string> {
  // 检查screenshots配置
  const screenshotsEnabled = Boolean(config.screenshots && (config.screenshots as any).enabled);
  ...
  try {
    ...
    await page.screenshot({
      path: filePath,
      fullPage: true,
      type: config.screenshots.format as 'png' | 'jpeg',
    });
...
async function ensureDirectoryExists(directory: string): Promise<void> {
  try {
    ...
    throw new McpError(
      ...
      '无法创建目录',
      { directory, originalError: error.message }
    );
  }
}
 ...
 * 获取截图数据
 */
export async function getScreenshotData(path: string): Promise<string> {
  // 检查文件是否存在
...
// 启动定期截图清理
let screenshotCleanupInterval: NodeJS.Timeout | null = null;

// 检查screenshots配置
const screenshotsEnabled = Boolean(config.screenshots && (config.screenshots as any).enabled);
if (screenshotsEnabled && config.screenshots.cleanupInterval > 0) {
  screenshotCleanupInterval = setInterval(
    cleanupOldScreenshots,
    config.screenshots.cleanupInterval
  );
}
